...
abstract class CopyPropagation {
  ...
  /** A *Ref class generated by lambda lifting. */
  case class RefCell(l: Location) extends Value
          ...
          if (isStatic)
            ...
            val v1 = in.stack match {
              case (r @ Record(cls, bindings)) :: xs =>
                log("\t Deref %s in record %s".format(field, r))
                Deref(Field(r, field))
...
//              if (method.isPrimaryConstructor) {
    ...
    private def getBindingsForPrimaryCtor(in: copyLattice.State, ctor: Symbol): Map[Symbol, Value] = {
      val paramAccessors = if (definitions.refClass.values.toSet(ctor.owner)) {
          log("..considering Ref class")
          List(ctor.owner.info.member(nme.elem))
        } else
          ctor.owner.constrParamAccessors
